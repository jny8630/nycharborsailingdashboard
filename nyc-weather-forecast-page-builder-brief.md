...
> Written 2026-06-24 to brief the build of `weather.html`, generated from a
> research session (hence the `[cite:NN]` markers, which point at that session's
> sources and are not resolvable here). It records *why* the page exists; it is
> not maintained. **`weather.html` is the authority** — do not treat anything
> below as current.
>
> Known drift since writing: the thunderstorm-risk requirement specified
> Open-Meteo, whose `thunderstorm_probability` is a DWD ICON-D2 field that
> returns null everywhere in the US — the strip built to this spec never once
> rendered. Thunder risk now comes from the NWS OKX gridpoint's
> `probabilityOfThunder` (2026-07-31). The "future work" section below proposing
> Google Weather API via a GitHub Actions cron was never acted on.
...
## Unimplemented Items (added 2026-06-24)

DRAFT — requires human review

Nothing below this line has been built yet. These are research items and layout changes captured from user and member feedback for future sessions.

### Layout Changes (from member feedback)

#### A. Move forecast to top of index page
A member's core feedback: "I usually check what's ahead of us." The index page should lead with forecast information, not recent observations. Forecast-first, then current conditions.

#### B. Remove B&G wind plot from index page
Move the B&G wind plot to the dedicated wind page (wind.html), which already has the longer 12-hour wind history chart. The wind page becomes the single home for all B&G / wind-direction detail. Index page gets cleaner.

#### C. Add NOAA Mariner forecast summary to index page
The card that summarizes the NOAA Mariner forecast in plain language should live on the index page (not buried on a subpage). This supports the "what's ahead" priority.

#### D. Extend Robbins Reef wind history chart
Currently shows ~2 hours of wind history. Investigate extending to ~3 hours to give slightly more context on recent wind trends. Research whether the NOAA data source supports a longer lookback without additional API calls.

### Research Items

### 1. TRMM Wind Model Data Source

A club member requested the TRMM (Tropical Rainfall Measuring Mission) wind model.

**Research needed:**
- Find a free, open API or data source for TRMM wind data
- TRMM satellite was decommissioned in 2015; successor is GPM (Global Precipitation Measurement) — likely need to investigate GPM instead
- Determine relevance for real-time NYC Harbor conditions vs. offshore/tropical use
- Check NASA GES DISC or similar for a usable feed

### 2. Lightning / Thunderstorm Activity Card

Add a dashboard card showing lightning strikes or thunderstorm activity near the upper NYC Harbor.

**Research needed:**
- Find free lightning strike data sources (e.g. Blitzortung.org is the main open-source option)
- Explore Windy’s additional thunderstorm views beyond the radar we already embed (thunderstorm probability, CAPE, lightning density)
- Determine if Windy thunderstorm layers are available as free iframe embeds or require API access
- Consider whether this replaces or complements the existing radar card

### 3. Google Weather AI Forecasting (GraphCast / WeatherNext)

Explore Google’s AI-powered weather forecasting for hyperlocal thunderstorm prediction in the upper harbor.

**Key constraint:** API call volume must be decoupled from page refreshes by club members.

**Proposed architecture:**
- GitHub Actions cron job (~1x/hour) fetches forecast from Google Weather API
- Writes result as static JSON committed to the repo or hosted on GitHub Pages
- Dashboard reads from the static file — unlimited page refreshes, zero additional API calls
- Google free tier allows ~10,000 calls/month; hourly refresh = ~720 calls/month, well within limit

**Research needed:**
- Confirm Google Weather API availability and free-tier limits for personal use
- Determine available forecast data (thunderstorm probability, precipitation, wind)
- Check geographic resolution for upper harbor relevance
- Design GitHub Actions workflow for hourly static refresh
